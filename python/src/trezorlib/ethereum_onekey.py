...
def get_field_type(type_name: str, types: dict) -> messages.EthereumFieldTypeChargerWallet:
    data_type = None
    size = None
    entry_type = None
    struct_name = None
    ...
    if is_array(type_name):
        data_type = messages.EthereumDataTypeChargerWallet.ARRAY
        ...
        if entry_type.data_type == messages.EthereumDataTypeChargerWallet.ARRAY:
            raise NotImplementedError("Nested arrays are not supported")
    elif type_name.startswith("uint"):
        data_type = messages.EthereumDataTypeChargerWallet.UINT
        size = get_byte_size_for_int_type(type_name)
    elif type_name.startswith("int"):
        data_type = messages.EthereumDataTypeChargerWallet.INT
        size = get_byte_size_for_int_type(type_name)
    elif type_name.startswith("bytes"):
        data_type = messages.EthereumDataTypeChargerWallet.BYTES
        size = None if type_name == "bytes" else parse_type_n(type_name)
    elif type_name == "string":
        data_type = messages.EthereumDataTypeChargerWallet.STRING
    elif type_name == "bool":
        data_type = messages.EthereumDataTypeChargerWallet.BOOL
    elif type_name == "address":
        data_type = messages.EthereumDataTypeChargerWallet.ADDRESS
    elif type_name in types:
        data_type = messages.EthereumDataTypeChargerWallet.STRUCT
        size = len(types[type_name])
        struct_name = type_name
    else:
        raise ValueError(f"Unsupported type name: {type_name}")

    return messages.EthereumFieldTypeChargerWallet(
        data_type=data_type,
        size=size,
...
@expect(messages.EthereumAddressChargerWallet, field="address", ret_type=str)
def get_address(
    client: "TrezorClient", n: "Address", show_display: bool = False
) -> "MessageType":
    return client.call(
        messages.EthereumGetAddressChargerWallet(address_n=n, show_display=show_display)
    )


@expect(messages.EthereumPublicKeyChargerWallet)
def get_public_node(
    client: "TrezorClient", n: "Address", show_display: bool = False
) -> "MessageType":
    return client.call(
        messages.EthereumGetPublicKeyChargerWallet(address_n=n, show_display=show_display)
    )
...
def sign_tx(
    ...
    msg = messages.EthereumSignTxChargerWallet(
        address_n=n,
    ...
    assert isinstance(response, messages.EthereumTxRequestChargerWallet)

    while response.data_length is not None:
        data_length = response.data_length
        data, chunk = data[data_length:], data[:data_length]
        response = client.call(messages.EthereumTxAckChargerWallet(data_chunk=chunk))
        assert isinstance(response, messages.EthereumTxRequestChargerWallet)
...
def sign_tx_eip1559(
    ...
    *,
    ...
    access_list: Optional[List[messages.EthereumAccessListChargerWallet]] = None,
) -> Tuple[int, bytes, bytes]:

    length = len(data)
    data, chunk = data[1024:], data[:1024]
    msg = messages.EthereumSignTxEIP1559ChargerWallet(
        address_n=n,
        nonce=int_to_big_endian(nonce),
        gas_limit=int_to_big_endian(gas_limit),
        value=int_to_big_endian(value),
...
@expect(messages.EthereumMessageSignatureChargerWallet)
def sign_message(
    client: "TrezorClient", n: "Address", message: AnyStr
) -> "MessageType":
    return client.call(
        messages.EthereumSignMessageChargerWallet(
            address_n=n, message=prepare_message_bytes(message)
        )
    )


@expect(messages.EthereumTypedDataSignatureChargerWallet)
def sign_typed_data(
    client: "TrezorClient",
    n: "Address",
    data: Dict[str, Any],
    *,
    ...
    request = messages.EthereumSignTypedDataChargerWallet(
        address_n=n,
    ...
    while isinstance(response, messages.EthereumTypedDataStructRequestChargerWallet):
        struct_name = response.name

        members: List["messages.EthereumStructMemberChargerWallet"] = []
        for field in types[struct_name]:
            field_type = get_field_type(field["type"], types)
            struct_member = messages.EthereumStructMemberChargerWallet(
                type=field_type,
                name=field["name"],
            )
            members.append(struct_member)

        request = messages.EthereumTypedDataStructAckChargerWallet(members=members)
        response = client.call(request)

    # Sending the whole message that should be signed
    while isinstance(response, messages.EthereumTypedDataValueRequestChargerWallet):
        ...
        request = messages.EthereumTypedDataValueAckChargerWallet(value=encoded_data)
        response = client.call(request)
...
def verify_message(
    ...
    try:
        resp = client.call(
            messages.EthereumVerifyMessageChargerWallet(
                address=address,
                signature=signature,
...
def sign_typed_data_hash(
    ...
    return client.call(
        messages.EthereumSignTypedHashChargerWallet(
            address_n=n,
