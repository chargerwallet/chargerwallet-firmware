...
class TrezorClient:
    ...
    @session
    def refresh_chargerwallet_features(self) -> messages.ChargerwalletFeatures:
        """Reload features from the device.

        Should be called after changing settings or performing operations that affect
        device state.
        """
        resp = self.call_raw(messages.ChargerwalletGetFeatures())
        if not isinstance(resp, messages.ChargerwalletFeatures):
            raise exceptions.TrezorException("Unexpected response to GetFeatures")
        return resp
