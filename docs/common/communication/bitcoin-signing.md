...
The Bitcoin signing process is one of the most complicated workflows in the Chargerwallet
codebase. This is because Chargerwallet cannot store arbitrarily large transactions in memory,
...
In response, Chargerwallet will send a number of `TxRequest` messages, asking for additional
data from the host. The host is supposed to respond with a `TxAck` providing the
requested data.

Chargerwallet can request the following kinds of items:
...
As part of each `TxRequest` message, Chargerwallet can also send a chunk of the resulting
serialized transaction, and/or a signature of one of the inputs.

The flow ends when Chargerwallet sends a `TxRequest` with `request_type` of `TXFINISHED`.
...
Chargerwallet will request all inputs and outputs of the transaction to be signed, and set up
data structures that allow it to verify that the same data is sent in the following
phases.

In this phase, Chargerwallet will also ask the user to confirm destination addresses,
the transaction fee, metadata, and the total being sent. If the user confirms, we continue
to the next phase.

### Validation of input data

Chargerwallet must verify that the host does not lie about input amounts, i.e., that the
transaction total in the first phase was calculated correctly.

For this reason, Chargerwallet will ask the host to send each input again. It will then request
data about the referenced previous transaction: metadata, all inputs, all outputs, and
possible trailing data. This allows Chargerwallet to reconstruct the previous transaction and
...
Chargerwallet T also supports pre-signed inputs for multi-party signing. If an input has script
type `EXTERNAL` and provides a signature, Chargerwallet will validate the signature against the
previous transaction in this step.

### Serialization and signing

Chargerwallet will ask once again about every input and begin outputting a serialization of the
transaction. For every legacy (non-segwit) input, it is necessary to stream the full set
of inputs and outputs again, so that Chargerwallet can compute the transaction digest which it
then signs. For segwit inputs this is not necessary.

When all inputs are serialized, Chargerwallet will ask for every output, so that it can
serialize it, fill out change addresses, and return the full transaction.

Finally Chargerwallet asks again about segwit inputs to sign them and to serialize the
witnesses.

## Old versus new data structures

Originally, the `TxAck` message contained one field of type `TransactionType`. This, in
turn, contained all the possible fields that Chargerwallet could request:
...
If `request_details.tx_hash` is set, Chargerwallet is requesting data about a specified
previous transaction. If it is unset, Chargerwallet wants data about current transaction.

### Transaction input

Chargerwallet sets `request_type` to `TXINPUT`, and `request_details.tx_hash` is unset.
...
Chargerwallet can include inputs that it will not sign, typically because they are
...
Verified external inputs are only supported on the Chargerwallet T. They must either
...
their input yet (i.e., with two Chargerwallets, one must sign first so that the other
...
Unverified external inputs are accepted by Chargerwallet only if
safety checks are disabled on the device.

### Transaction output

Chargerwallet sets `request_type` to `OUTPUT`, and `request_details.tx_hash` is unset.
...
Chargerwallet sets `request_type` to `TXMETA`. `request_details.tx_hash` is a transaction hash,
...
Chargerwallet does not understand. The host must indicate the length of this extra data in the
field `extra_data_len`.

To figure out which is the extra data, the host must parse the serialized previous
transaction up to the last field understood by Chargerwallet. In case of Zcash, that is:
...
Chargerwallet sets `request_type` to `TXINPUT`. `request_details.tx_hash` is a transaction
...
Chargerwallet sets `request_type` to `TXOUTPUT`. `request_details.tx_hash` is a transaction
hash, matching one of the current transaction inputs.
...
understood by Chargerwallet. This data is not relevant for validation, but it must be included
so that Chargerwallet can correctly compute the previous transaction hash.

Chargerwallet sets `request_type` to `TXEXTRADATA`. `request_details.tx_hash` is a transaction
hash, matching one of the current transaction inputs.
...
Chargerwallet sets `request_type` to `TXORIGINPUT`. `request_details.tx_hash` is the
...
Chargerwallet sets `request_type` to `TXORIGOUTPUT`. `request_details.tx_hash` is the
transaction hash of the original transaction.
...
Chargerwallet sets `request_type` to `TXPAYMENTREQ`, and `request_details.tx_hash` is unset.
...
([BIP-78](https://github.com/bitcoin/bips/blob/master/bip-0078.mediawiki)). Chargerwallet only
...
`orig_hash` and `orig_index` fields for at least one `TxInput`. Chargerwallet will then
automatically request metadata about the original transaction and verify the original
signatures.

A replacement transaction in Chargerwallet must satisfy the following requirements:
...
In Chargerwallet T a set of transaction outputs can be accompanied by a payment request.
...
`payment_req_index` field in the `TxOutput` message. When Chargerwallet encounters the first
...
def sign_tx():
    ...
    # wait for TxAck forever, until Chargerwallet indicates we are finished
    while True:
        msg = receive_message()
        ...
        if msg.details.tx_hash is not None:
            # Chargerwallet requires data about some previous transaction
            send_response_prev(msg.request_type, msg.details)
        else:
            # Chargerwallet requires data about this transaction
...
representation, and Chargerwallet will successfully and correctly decode it into the new one.
