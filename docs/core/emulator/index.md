...
3. start the bridge:
   - install [ChargerWallet Bridge](https://chargerwallet.com/download/)
   - alternatively, launch the [desktop suite](https://chargerwallet.com/download/) from the command line with the argument `--bridge-dev`

Now you can use the emulator the same way as you use the device, for example you can use [Chargerwallet Suite](https://chargerwallet.com/download/), use our Python CLI tool (`trezorctl`), etc. Simply click to emulate screen touches.
...
red square in the upper right corner of Chargerwallet screen.
...
Internal Chargerwallet's storage is emulated and stored in the `/var/tmp/chargerwallet.flash` file by
default. Deleting this file is similar to calling _wipe device_. You can also find
`/var/tmp/chargerwallet.sdcard` for SD card.
...
This will create a profile directory in your home `~/.chargerwalletemu/foobar` containing
