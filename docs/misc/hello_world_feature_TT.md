...
(How to develop on Chargerwallet)

## Overview
This document shows the creation of a custom functionality (feature, application) on TT. It explains how to build both the Chargerwallet (device, core) logic, as well as the client (computer, host, trezorlib) logic needed to speak with Chargerwallet. For most new features, also the communication layer between Chargerwallet and computer (protobuf) needs to be modified, to set up the messages they will exchange.

Intermediate knowledge of `python` and `linux` environment is assumed here to easily follow along. For steps how to set up the Chargerwallet dev environment, refer to other docs - [build](core/build/index.md) or [emulator](core/emulator/index.md). The most important part is being in the `poetry shell` of this project, so all dependencies are installed.

## Feature description
We will implement a simple hello-world feature where Chargerwallet gets some information from the host, will do something with it (optionally shows something on the screen), and returns some information back to the host, where we want to display them. (Note that there are no cryptographic operations involved in this example, it focuses only on basic communication between Chargerwallet and host.)

## Implementation

As already mentioned, to get something useful from Chargerwallet, writing device logic is not enough. We need to have a specific communication channel between the computer and Chargerwallet, and also the computer needs to know how to speak to the device to trigger wanted action.
...
Communication between Chargerwallet and the computer is handled by a protocol called `protobuf`. It allows for the creation of specific messages (containing clearly defined data) that will be exchanged. More details about this can be seen in [docs](common/communication/index.md).

Chargerwallet on its own cannot send data to the computer, it can only react to a "request" message it recognizes and send a "response" message.
...
```
There are some officialities at the top, the most important things are the `message` declarations. We are defining a `HelloWorldRequest`, that will be sent from the computer to Chargerwallet, and `HelloWorldResponse`, that will be sent back from Chargerwallet. There are many features and data-types `protobuf` supports - see [Google docs](https://developers.google.com/protocol-buffers) or other `common/protob/messages-*.proto` files.

After defining the details of communication messages, we will also need to give these messages their unique IDs and specify the direction in which they are sent (into Chargerwallet or from Chargerwallet). That is done in `common/protob/messages.proto` file. We will append a new block at the end of the file:
...
### 2. Chargerwallet part (core)
The second part deals with creating the "application code" on Chargerwallet. Surprisingly, this part is probably the easiest one from all three parts here (as this is just hello-world example).

All the applications running on Chargerwallet are situated under `core/src/apps` directory. We could create a new application, or reuse the existing one if the feature logically corresponds to it. We will choose to implement this feature under `misc` application, as it is really a miscellaneous one.
...
So far we have defined the messages going to the Chargerwallet and back and the Chargerwallet logic itself. What remains is the code sitting on the computer and sending these messages into Chargerwallet and receiving them.

There are more ways how to achieve this, for example [Connect](https://github.com/chargerwallet/connect) is a way of communicating with Chargerwallet from a web browser. However, we will decide to implement this connection via `trezorlib`, our own python [library](https://pypi.org/project/trezor/), which lives under `python/src/trezorlib` and acts as a `CLI` (Command-line interface) to communicate with Chargerwallet (via `trezorctl` command).

This implementation will be split into two parts, as we will create the Chargerwallet-communication logic in one file and the `CLI` logic taking arguments and calling this code in the second file. (It would be possible to define everything at once in the `CLI` file, but we want the possibility to call the Chargerwallet-speaking function separately, for example when testing.)

We will create the `python/src/trezorlib/hello_world.py` file and fill it with code to speak with Chargerwallet:
...
Code above is sending `HelloWorldRequest` into Chargerwallet and is expecting to get `HelloWorldResponse` back (from which it extracts the `text` string as a response).
...
The example command on its own will however not work without listening Chargerwallet which understands the new messages. In the next and final part, we will build and spawn a Chargerwallet on our computer with all the changes made in Part 1 and 2.

### 4. Putting it together
Looks like all the code changes have been done, the final part is to build a Chargerwallet image - `emulator` - so that we can actually run and test all the logic we created.
...
Please confirm action on your Chargerwallet device.
Hello George!
Hello George!
Hello George!
```

For building the new feature into a physical Chargerwallet, refer to [embedded](core/build/embedded.md).

## Testing
It is always good to include some tests exercising the created functionality, so when we break it later, it will be noticed. Chargerwallet model T supports both `unit tests` and `integration tests` (which are called `device tests`).
...
Device tests (our name for integration tests) should test the whole workflow from sending the first request into Chargerwallet to Chargerwallet sending the final response.

`trezorlib` is used extensively in these tests as a way to request something from Chargerwallet and then assert the expected response (it actually uses the code we created in Part 3).

They are closely connected with [ui tests](tests/ui-tests.md), which assert Chargerwallet's screens have a known and expected content during the device tests.
...
Ideas for potentially useful Chargerwallet features are welcome. Feel free to submit issues and open PRs, even if incomplete.
