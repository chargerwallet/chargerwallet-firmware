# Chargerwallet Core Boot Stages

Chargerwallet T initialization is split into two stages. See [Memory Layout](memory.md) for info about in which sectors each stage is stored.
...
* There is a tool called [headertool.py](https://github.com/chargerwallet/firmware/blob/master/core/tools/headertool.py) which checks validity
  of the bootloader/firmware images including their headers.

## Bootloader Format

Chargerwallet Core (second stage) bootloader consists of 2 parts:
...
| 0x03BF | 1      | sigmask | Chargerwallet signature indexes (bitmap) |
| 0x03C0 | 64     | sig | Chargerwallet aggregated signature of the bootloader header |

## Firmware Format

Chargerwallet Core firmware consists of 3 parts:
...
| ?      | 1      | sigmask | Chargerwallet signature indexes (bitmap) |
| ?      | 64     | sig | Chargerwallet aggregated signature of the vendor header |
