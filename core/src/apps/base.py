...
def get_vendor():
    return "trezor.io" if storage.device.is_trezor_compatible() else "chargerwallet.com"
...
def get_features() -> Features:
    ...
    from trezor.enums import Capability, ChargerWalletDeviceType, ChargerWalletSeType
    ...
    f = Features(
        ...
        chargerwallet_version=utils.CHARGERWALLET_VERSION,
        ...
        chargerwallet_device_type=ChargerWalletDeviceType.TOUCH,
        chargerwallet_se_type=ChargerWalletSeType.SE608A,
        chargerwallet_board_version=utils.board_version(),
        chargerwallet_boot_version=utils.boot_version(),
        chargerwallet_firmware_version=utils.CHARGERWALLET_VERSION,
        chargerwallet_serial_no=storage_serial_no,
        chargerwallet_ble_name=uart.get_ble_name(),
        chargerwallet_ble_version=uart.get_ble_version(),
    )
...
def get_chargerwallet_features() -> ChargerwalletFeatures:
    from trezor.enums import ChargerWalletDeviceType, ChargerWalletSeType
    from trezor.messages import ChargerwalletFeatures
    from trezor import uart

    storage_serial_no = storage.device.get_serial()
    f = ChargerwalletFeatures(
        chargerwallet_device_type=ChargerWalletDeviceType.TOUCH,
        chargerwallet_serial_no=storage_serial_no,
        chargerwallet_se_type=ChargerWalletSeType.SE608A,
        chargerwallet_board_version=utils.board_version(),
        chargerwallet_board_hash=utils.board_hash(),
        chargerwallet_boot_version=utils.boot_version(),
        chargerwallet_boot_hash=utils.boot_hash(),
        chargerwallet_boot_build_id=utils.boot_build_id(),
        chargerwallet_firmware_version=utils.CHARGERWALLET_VERSION,
        chargerwallet_firmware_build_id=utils.BUILD_ID[-7:].decode(),
        chargerwallet_firmware_hash=utils.chargerwallet_firmware_hash(),
        chargerwallet_ble_name=uart.get_ble_name(),
        chargerwallet_ble_version=uart.get_ble_version(),
        chargerwallet_ble_build_id=uart.get_ble_build_id(),
        chargerwallet_ble_hash=uart.get_ble_hash(),
    )
...
async def handle_ChargerwalletGetFeatures(
    ctx: wire.Context, msg: ChargerwalletGetFeatures
) -> ChargerwalletFeatures:
    return get_chargerwallet_features()
...
ALLOW_WHILE_LOCKED = (
    ...
    MessageType.ChargerwalletGetFeatures,
    MessageType.Cancel,
...
def boot() -> None:
    ...
    workflow_handlers.register(MessageType.ChargerwalletGetFeatures, handle_ChargerwalletGetFeatures)
