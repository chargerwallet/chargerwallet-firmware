...
def find_message_handler_module(msg_type: int) -> str:
    ...
    if not utils.BITCOIN_ONLY:
        ...
        # ethereum chargerwallet
        if msg_type == MessageType.EthereumGetAddressChargerWallet:
            return "apps.ethereum.chargerwallet.get_address"
        if msg_type == MessageType.EthereumGetPublicKeyChargerWallet:
            return "apps.ethereum.chargerwallet.get_public_key"
        if msg_type == MessageType.EthereumSignTxChargerWallet:
            return "apps.ethereum.chargerwallet.sign_tx"
        if msg_type == MessageType.EthereumSignTxEIP1559ChargerWallet:
            return "apps.ethereum.chargerwallet.sign_tx_eip1559"
        if msg_type == MessageType.EthereumSignMessageChargerWallet:
            return "apps.ethereum.chargerwallet.sign_message"
        if msg_type == MessageType.EthereumVerifyMessageChargerWallet:
            return "apps.ethereum.chargerwallet.verify_message"
        if msg_type == MessageType.EthereumSignTypedDataChargerWallet:
            return "apps.ethereum.chargerwallet.sign_typed_data"
        if msg_type == MessageType.EthereumSignTypedHashChargerWallet:
            return "apps.ethereum.chargerwallet.sign_typed_data_hash"
