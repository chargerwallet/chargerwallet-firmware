...
if TYPE_CHECKING:
    from trezor.messages import EthereumFieldType, EthereumFieldTypeChargerWallet
...
def get_type_name_chargerwallet(field: EthereumFieldTypeChargerWallet) -> str:
    """Create a string from type definition (like uint256 or bytes16)."""
    from trezor.enums import EthereumDataTypeChargerWallet

    data_type = field.data_type
    size = field.size

    TYPE_TRANSLATION_DICT = {
        EthereumDataTypeChargerWallet.UINT: "uint",
        EthereumDataTypeChargerWallet.INT: "int",
        EthereumDataTypeChargerWallet.BYTES: "bytes",
        EthereumDataTypeChargerWallet.STRING: "string",
        EthereumDataTypeChargerWallet.BOOL: "bool",
        EthereumDataTypeChargerWallet.ADDRESS: "address",
    }

    if data_type == EthereumDataTypeChargerWallet.STRUCT:
        assert field.struct_name is not None  # validate_field_type
        return field.struct_name
    elif data_type == EthereumDataTypeChargerWallet.ARRAY:
        assert field.entry_type is not None  # validate_field_type
        type_name = get_type_name_chargerwallet(field.entry_type)
        if size is None:
            return f"{type_name}[]"
        else:
            return f"{type_name}[{size}]"
    elif data_type in (EthereumDataTypeChargerWallet.UINT, EthereumDataTypeChargerWallet.INT):
        assert size is not None  # validate_field_type
        return TYPE_TRANSLATION_DICT[data_type] + str(size * 8)
    elif data_type == EthereumDataTypeChargerWallet.BYTES:
        if size:
            return TYPE_TRANSLATION_DICT[data_type] + str(size)
        else:
            return TYPE_TRANSLATION_DICT[data_type]
