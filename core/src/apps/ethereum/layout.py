...
from trezor.enums import ButtonRequestType, EthereumDataType, EthereumDataTypeChargerWallet
...
async def should_show_struct(
    ctx: Context,
    description: str,
    data_members: list[EthereumStructMember] | list[EthereumStructMemberChargerWallet],
...
async def confirm_typed_value_chargerwallet(
    ctx: Context,
    name: str,
    value: bytes,
    parent_objects: list[str],
    field: EthereumFieldTypeChargerWallet,
    array_index: int | None = None,
) -> None:
    type_name = get_type_name_chargerwallet(field)
    ...
    if field.data_type in (
        EthereumDataTypeChargerWallet.ADDRESS,
        EthereumDataTypeChargerWallet.BYTES,
    ):
        await confirm_blob(
            ctx,
            "confirm_typed_value",
