...
from trezor.enums import EthereumDataTypeChargerWallet as EthereumDataType
from trezor.lvglui.i18n import gettext as _, keys as i18n_keys
from trezor.messages import (
    EthereumFieldTypeChargerWallet as EthereumFieldType,
    EthereumSignTypedDataChargerWallet as EthereumSignTypedData,
    EthereumTypedDataSignatureChargerWallet as EthereumTypedDataSignature,
    EthereumTypedDataStructAckChargerWallet as EthereumTypedDataStructAck,
    EthereumTypedDataStructRequestChargerWallet as EthereumTypedDataStructRequest,
    EthereumTypedDataValueAckChargerWallet as EthereumTypedDataValueAck,
    EthereumTypedDataValueRequestChargerWallet as EthereumTypedDataValueRequest,
)
...
from ..helpers import (
    ...
    get_type_name_chargerwallet,
)
from ..layout import (
    confirm_empty_typed_message,
    confirm_typed_data_final,
    confirm_typed_value_chargerwallet,
    should_show_array,
    should_show_struct,
)
...
class TypedDataEnvelope:
    ...
    def encode_type(self, primary_type: str) -> bytes:
        ...
        for type_name in [primary_type] + sorted(deps):
            ...
            fields = ",".join(
                f"{get_type_name_chargerwallet(m.type)} {m.name}" for m in members
            )
            ...
            if field_type.data_type == EthereumDataType.STRUCT:
                ...
                if show_data:
                    show_array = await should_show_array(
                        ctx=self.ctx,
                        parent_objects=current_parent_objects,
                        data_type=get_type_name_chargerwallet(entry_type),
                        size=array_size,
                    )
                else:
                    show_array = False
                ...
                for i in range(array_size):
                    ...
                    if entry_type.data_type == EthereumDataType.STRUCT:
                        ...
                        if show_array:
                            await confirm_typed_value_chargerwallet(
                                ctx=self.ctx,
                                name=field_name,
                                value=value,
                                parent_objects=parent_objects,
