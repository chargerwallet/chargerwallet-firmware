...
async def confirm_path_warning(
    ...
    screen.btn_yes.enable(bg_color=lv_colors.CHARGERWALLET_YELLOW, text_color=lv_colors.BLACK)
    await raise_if_cancelled(
        interact(
            ctx,
            screen,
...
async def _show_modal(
    ...
    if btn_yes_bg_color:
        screen.btn_yes.enable(bg_color=btn_yes_bg_color or lv_colors.CHARGERWALLET_GREEN)
    await raise_if_cancelled(
        interact(
            ctx,
            screen,
...
async def show_chargerwallet_app_guide():
    if not __debug__:
...
async def confirm_del_wallpaper(ctx, confirm_callback):
    ...
    confirm_screen.btn_yes.enable(
        bg_color=lv_colors.CHARGERWALLET_RED_1, text_color=lv_colors.BLACK
    )
