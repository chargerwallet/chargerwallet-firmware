...
class DeviceInfoManager:
    ...
    def preload_device_info(self):
        ...
        hex_hash = binascii.hexlify(chargerwallet_firmware_hash).decode("ascii")
        short_hash = hex_hash[:7]
        version = f"{version} [{chargerwallet_firmware_build_id}-{short_hash}]"
        ...
        hex_hash = binascii.hexlify(chargerwallet_boot_hash).decode("ascii")
        short_hash = hex_hash[:7]
        chargerwallet_boot_build_id = utils.boot_build_id()
        boot_version = f"{boot_version} [{chargerwallet_boot_build_id}-{short_hash}]"
