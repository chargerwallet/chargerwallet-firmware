...
class Slider(lv.slider):
    ...
    def __init__(self, parent, text, relative_y=-122) -> None:
        ...
        self.add_style(
            ...
            .bg_color(lv_colors.CHARGERWALLET_BLACK_4)
            .bg_opa(lv.OPA.COVER)
            .pad_all(3)
            .radius(2),
            lv.PART.MAIN,
        ...
        self.tips.add_style(
            ...
            .text_letter_space(-1)
            .text_color(lv_colors.CHARGERWALLET_WHITE_4),
            0,
        )
    ...
    def enable(self, enable: bool = True):
        if enable:
            ...
            self.set_style_bg_color(
                lv_colors.CHARGERWALLET_BLACK_4, lv.PART.MAIN | lv.STATE.DEFAULT
            )
            self.set_style_bg_color(
                lv_colors.CHARGERWALLET_BLACK_4, lv.PART.INDICATOR | lv.STATE.DEFAULT
            )
            self.arrow_img_src = Slider.SLIDER_ARROW_WHITE_IMG_SRC
            self.done_img_src = Slider.SLIDER_DONE_WHITE_IMG_SRC
            self.set_style_bg_color(
                lv_colors.CHARGERWALLET_RED_1, lv.PART.KNOB | lv.STATE.DEFAULT
            )
            self.set_style_border_color(lv_colors.CHARGERWALLET_GRAY, 0)
            self.tips.set_style_text_color(lv_colors.CHARGERWALLET_WHITE_4, 0)
        else:
            self.disable = True
            self.set_style_bg_color(
                lv_colors.CHARGERWALLET_BLACK_3, lv.PART.KNOB | lv.STATE.DEFAULT
            )
            self.set_style_border_color(lv_colors.CHARGERWALLET_GRAY_1, 0)
            self.tips.set_style_text_color(lv_colors.WHITE_2, 0)

    def change_knob_style(self, level):
        if level == 1:
            self.add_style(
                StyleWrapper().bg_color(lv_colors.CHARGERWALLET_YELLOW),
            ...
            self.add_style(
                StyleWrapper().bg_color(lv_colors.CHARGERWALLET_RED_1),
                lv.PART.KNOB | lv.STATE.DEFAULT,
            )
        ...
        if code == lv.EVENT.PRESSING:
            ...
            if dsc.part == lv.PART.MAIN:
                ...
                label_draw_dsc.color = (
                    lv_colors.WHITE_2 if self.disable else lv_colors.CHARGERWALLET_WHITE_4
                )
