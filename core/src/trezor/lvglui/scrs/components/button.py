...
class NormalButton(lv.btn):
    ...
    def disable(
        self, bg_color=lv_colors.CHARGERWALLET_BLACK_1, text_color=lv_colors.CHARGERWALLET_GRAY
    ) -> None:
        self.add_style(StyleWrapper().bg_color(bg_color).text_color(text_color), 0)
        self.clear_flag(lv.btn.FLAG.CLICKABLE)

    def enable(
        self, bg_color=lv_colors.CHARGERWALLET_BLACK, text_color=lv_colors.WHITE
    ) -> None:
        self.add_style(StyleWrapper().bg_color(bg_color).text_color(text_color), 0)
...
class ListItemBtn(lv.btn):
    def __init__(
        ...
        self.add_style(
            StyleWrapper()
            .bg_color(lv_colors.CHARGERWALLET_GRAY_3 if has_bgcolor else lv_colors.BLACK)
            .bg_opa(lv.OPA.COVER)
            .min_height(94)
            .text_font(font_PJSBOLD30)
        ...
        self.add_style(
            StyleWrapper().bg_color(lv_colors.CHARGERWALLET_BLACK_2).transform_height(-2)
            # .transform_width(-4)
    ...
    def set_checked(self) -> None:
        if self.img_right.has_flag(lv.obj.FLAG.HIDDEN):
            ...
            if not self.unique_bg:
                self.add_style(StyleWrapper().bg_color(lv_colors.CHARGERWALLET_GRAY_2), 0)
...
class ListItemBtnWithSwitch(lv.btn):
    def __init__(self, parent, text: str) -> None:
        ...
        self.add_style(
            StyleWrapper()
            # .bg_color(lv_colors.CHARGERWALLET_BLACK_2)
        ...
        self.switch.add_style(
            StyleWrapper().bg_color(lv_colors.CHARGERWALLET_GRAY).radius(4), 0
        )
        self.switch.add_style(
            StyleWrapper().bg_color(lv_colors.CHARGERWALLET_GREEN).radius(4),
