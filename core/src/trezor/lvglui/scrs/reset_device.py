...
class MnemonicDisplay(FullSizeWindow):
    def __init__(self, title: str, mnemonics: Sequence[str]):
        ...
        # self.panel.set_style_bg_color(lv_colors.CHARGERWALLET_GRAY_3, 0)
    ...
    #                 self.btn_yes.enable(bg_color=lv_colors.CHARGERWALLET_GREEN)
    #             else:
...
class BackupTips(FullSizeWindow):
    ...
    def on_event(self, event_obj: lv.event_t):
        ...
        if code == lv.EVENT.VALUE_CHANGED:
            ...
            if self.cb_cnt == 3:
                self.btn_yes.enable(
                    bg_color=lv_colors.CHARGERWALLET_GREEN, text_color=lv_colors.BLACK
                )
            elif self.cb_cnt < 3:
                self.btn_yes.disable()
