...
if [[ $RUN_TEST_EMU > 0 ]]; then
  ...
  mv "${TREZOR_PROFILE_DIR}/chargerwallet.flash" "${TREZOR_PROFILE_DIR}/chargerwallet.flash.bkp" 2>/dev/null
  mv "${TREZOR_PROFILE_DIR}/onkey.sdcard" "${TREZOR_PROFILE_DIR}/chargerwallet.sdcard.bkp" 2>/dev/null
