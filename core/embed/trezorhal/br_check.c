...
static int chargerwallet_known_boardloader(const uint8_t *hash) {
  if (0 ==
      memcmp(hash,
...
char *get_boardloader_version(void) {
  ...
  if (strlen(boardloader_version) == 0) {
    ...
    chargerwallet_known_boardloader(hash);
  }
...
uint8_t *get_firmware_hash(void) {
  static uint8_t chargerwallet_firmware_hash[32] = {0};
  static bool chargerwallet_firmware_hash_cached = false;
  if (!chargerwallet_firmware_hash_cached) {
    ...
    if (vhdr->magic != 0x56544B4F || hdr->magic != FIRMWARE_IMAGE_MAGIC)
      return chargerwallet_firmware_hash;
    ...
    sha256_Update(&context,
                  ...
                  outer_firmware_len);
    sha256_Final(&context, chargerwallet_firmware_hash);

    chargerwallet_firmware_hash_cached = true;
  }
