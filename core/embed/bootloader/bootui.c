...
#define COLOR_BL_DANGER RGB16(0xFF, 0x11, 0x00)    // chargerwallet red
...
void ui_logo_center(void) {
  display_image(203, 56, 74, 74, toi_icon_chargerwallet_74x74 + 12,
                sizeof(toi_icon_chargerwallet_74x74) - 12);
}
...
#if PRODUCTION_MODEL == 'H'
  const uint32_t fw_version = hdr->chargerwallet_version;
#else
...
void ui_screen_welcome_third(void) {
  ...
  display_icon((DISPLAY_RESX - 180) / 2, (DISPLAY_RESY - 30) / 2 - 5, 180, 30,
               ...
               COLOR_WELCOME_FG, COLOR_WELCOME_BG);
  display_text_center(120, 220, "Go to chargerwallet.com", -1, FONT_NORMAL,
                      COLOR_WELCOME_FG, COLOR_WELCOME_BG);
}
...
void ui_screen_firmware_info(const vendor_header *const vhdr,
  ...
  if (vhdr && hdr) {
#if PRODUCTION_MODEL == 'H'
    ver_str = format_ver("Firmware %d.%d.%d by", (hdr->chargerwallet_version));
#else
  ...
  display_text_center(120, 220, "Go to chargerwallet.com", -1, FONT_NORMAL, COLOR_BL_FG,
                      COLOR_BL_BG);
}
...
#if PRODUCTION_MODEL == 'H'
  const char *ver_str = format_ver("to version %d.%d.%d?", hdr->chargerwallet_version);
#else
...
void ui_screen_install_confirm_newvendor_or_downgrade_wipe(
  ...
  const char *ver_str = format_ver("%d.%d.%d", current_hdr.chargerwallet_version);
  display_text_right(MAX_DISPLAY_RESX / 2 - 25, 320, ver_str, -1, FONT_NORMAL,
                     COLOR_BL_SUBTITLE, COLOR_BL_BG);
  ver_str = format_ver("%d.%d.%d", hdr->chargerwallet_version);
...
void ui_wipe_confirm(const image_header *const hdr) {
  ui_title_update();
  ui_logo_center();
  // if (hdr && (hdr->chargerwallet_version != 0)) {
  //   const char *ver_str = format_ver("v%d.%d.%d", (hdr->chargerwallet_version));
...
void ui_install_confirm(image_header *current_hdr,
  ...
  display_text_center(DISPLAY_RESX / 2, 190, "System Update", -1,
                      FONT_PJKS_BOLD_38, COLOR_BL_FG, COLOR_BL_BG);
  display_text_center(DISPLAY_RESX / 2, 240, "Install firmware by ChargerWallet?", -1,
                      FONT_NORMAL, COLOR_BL_SUBTITLE, COLOR_BL_BG);

  const char *ver_str = format_ver("%d.%d.%d", current_hdr->chargerwallet_version);
  display_text_right(DISPLAY_RESX / 2 - 25, 320, ver_str, -1, FONT_NORMAL,
                     COLOR_BL_SUBTITLE, COLOR_BL_BG);
  ver_str = format_ver("%d.%d.%d", new_hdr->chargerwallet_version);
...
void ui_bootloader_view_details(const image_header *const hdr) {
  ...
  display_text(offset_x, offset_y, "ChargerWallet Touch", -1, FONT_NORMAL,
...
void ui_bootloader_factory(void) {
  display_image(203, 108, 74, 74, toi_icon_chargerwallet_74x74 + 12,
                sizeof(toi_icon_chargerwallet_74x74) - 12);
