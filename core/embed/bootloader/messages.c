...
static void send_msg_features(uint8_t iface_num,
  ...
  if (device_is_factory_mode()) {
    ...
    MSG_SEND_ASSIGN_STRING(vendor, "chargerwallet.com");
    ...
    if (vhdr && hdr) {
      ...
      const char *ver_str = format_ver("%d.%d.%d", hdr->chargerwallet_version);
      MSG_SEND_ASSIGN_STRING_LEN(chargerwallet_version, ver_str, strlen(ver_str));
      MSG_SEND_ASSIGN_STRING_LEN(chargerwallet_firmware_version, ver_str,
                                 strlen(ver_str));
    } else {
    ...
    if (ble_name_state()) {
      ...
      MSG_SEND_ASSIGN_STRING_LEN(chargerwallet_ble_name, ble_get_name(), BLE_NAME_LEN);
    }
    if (ble_ver_state()) {
      char *ble_version = ble_get_ver();
      MSG_SEND_ASSIGN_STRING_LEN(ble_ver, ble_version, strlen(ble_version));
      MSG_SEND_ASSIGN_STRING_LEN(chargerwallet_ble_version, ble_version,
                                 strlen(ble_version));
    }
    if (ble_switch_state()) {
    ...
    MSG_SEND_ASSIGN_STRING_LEN(chargerwallet_board_version, board_version,
                               strlen(board_version));

    char *boot_version = (VERSTR(VERSION_MAJOR) "." VERSTR(
        VERSION_MINOR) "." VERSTR(VERSION_PATCH));
    MSG_SEND_ASSIGN_STRING_LEN(chargerwallet_boot_version, boot_version,
                               strlen(boot_version));

    MSG_SEND_ASSIGN_VALUE(chargerwallet_device_type, ChargerWalletDeviceType_TOUCH);
    MSG_SEND_ASSIGN_VALUE(chargerwallet_se_type, ChargerWalletSeType_SE608A);
  }
...
static void send_msg_features_ex(uint8_t iface_num,
                                 const vendor_header *const vhdr,
                                 const image_header *const hdr) {
  MSG_SEND_INIT(ChargerwalletFeatures);
  ...
  if (vhdr && hdr) {
    ...
    MSG_SEND_ASSIGN_BYTES(chargerwallet_firmware_hash, fimware_hash, 32);
  }
  if (ble_name_state()) {
    MSG_SEND_ASSIGN_STRING_LEN(chargerwallet_ble_name, ble_get_name(), BLE_NAME_LEN);
  }
  ...
  char *serial = NULL;
  if (device_get_serial(&serial)) {
    MSG_SEND_ASSIGN_STRING_LEN(chargerwallet_serial_no, serial, strlen(serial));
  }
  char *board_version = get_boardloader_version();
  MSG_SEND_ASSIGN_STRING_LEN(chargerwallet_board_version, board_version,
                             strlen(board_version));
  uint8_t *board_hash = get_boardloader_hash();
  MSG_SEND_ASSIGN_BYTES(chargerwallet_board_hash, board_hash, 32);
  ...
  MSG_SEND_ASSIGN_BYTES(chargerwallet_boot_hash, boot_hash, 32);
  MSG_SEND_ASSIGN_STRING_LEN(chargerwallet_boot_build_id, (char *)BUILD_COMMIT,
                             strlen((char *)BUILD_COMMIT));
  MSG_SEND_ASSIGN_VALUE(chargerwallet_device_type, ChargerWalletDeviceType_TOUCH);
  MSG_SEND_ASSIGN_VALUE(chargerwallet_se_type, ChargerWalletSeType_SE608A);

  MSG_SEND(ChargerwalletFeatures);
}
...
void process_msg_ChargerwalletGetFeatures(uint8_t iface_num, uint32_t msg_size,
                                   uint8_t *buf,
                                   const vendor_header *const vhdr,
                                   const image_header *const hdr) {
  MSG_RECV_INIT(ChargerwalletGetFeatures);
  MSG_RECV(ChargerwalletGetFeatures);
...
static void detect_installation(vendor_header *current_vhdr,
  ...
  *is_downgrade_wipe = secfalse;
  ...
  if (version_compare(new_hdr->chargerwallet_version, current_hdr->chargerwallet_version) <
      0) {
