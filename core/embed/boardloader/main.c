...
// clang-format off
static const uint8_t toi_icon_chargerwallet[] = {
    // magic
    'T', 'O', 'I', 'f',
...
#define BOARD_VERSION "ChargerWallet Boardloader 1.4.0\n"
...
int fatfs_check_res(void) {
  ...
  res = f_open(&fsrc, "/res/.CHARGERWALLET_RESOURCE", FA_READ);
  if (res != FR_OK) {
    f_unmount("");
  }
  return res;
...
int main(void) {
  ...
  if (startup_mode_flag != STAY_IN_BOARDLOADER_FLAG &&
    ...
    display_image((DISPLAY_RESX - 128) / 2, 190, 128, 128, toi_icon_chargerwallet + 12,
                  sizeof(toi_icon_chargerwallet) - 12);
    ...
    display_text_center(DISPLAY_RESX / 2, DISPLAY_RESY - 64,
                        "Powered by ChargerWallet", -1, FONT_NORMAL, COLOR_GRAY,
                        COLOR_BLACK);
