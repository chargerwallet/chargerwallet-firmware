...
void emmc_fs_format_status(char* str, size_t str_len)
{
    ...
    snprintf(
        str, str_len, 
        "\nis_inited=%s\nis_mounted_chargerwallet_data=%s\nis_mounted_user_data=%s\nfrom_function=%s\nff_call=%s\nff_result=%x\ndebug_extra=%s\n",
        emmc_wrapper_status.is_inited?"True":"False",
        emmc_wrapper_status.is_mounted_chargerwallet_data?"True":"False",
...
bool emmc_fs_recreate(bool partition_table, bool chargerwallet_data, bool user_data)
{
    // status update
    OKEMMC_DBG_STATUS_SETUP();

    // reset status
    emmc_wrapper_status.is_mounted_chargerwallet_data = false;
    ...
    if ( partition_table || chargerwallet_data )
    {
        ExecuteCheck_OKEMMC_SIMPLE(f_mkfs("0:", &mk_para, work, sizeof(work)));
        ExecuteCheck_OKEMMC_SIMPLE(f_mount(&emmc_wrapper_status.fs_obj_chargerwalletData, "0:", 1));

        // set label
        ExecuteCheck_OKEMMC_SIMPLE(f_setlabel("0:Chargerwallet Data"));
        ...
        ExecuteCheck_OKEMMC_SIMPLE(f_open(&res_flag, "0:res/.CHARGERWALLET_RESOURCE", FA_CREATE_NEW | FA_WRITE));
...
bool emmc_fs_mount(bool chargerwallet_data, bool user_data)
{
    // status update
    OKEMMC_DBG_STATUS_SETUP();
    ...
    if ( chargerwallet_data )
    {
        ExecuteCheck_OKEMMC_SIMPLE(f_mount(&emmc_wrapper_status.fs_obj_chargerwalletData, "0:", 1));
        emmc_wrapper_status.is_mounted_chargerwallet_data = true;
    }
    if ( user_data )
    {
        ExecuteCheck_OKEMMC_SIMPLE(f_mount(&emmc_wrapper_status.fs_obj_userData, "1:", 1));
...
bool emmc_fs_unmount(bool chargerwallet_data, bool user_data)
{
    // status update
    OKEMMC_DBG_STATUS_SETUP();
...
bool emmc_fs_fix_permission(bool chargerwallet_data, bool user_data)
{
    // status update
    OKEMMC_DBG_STATUS_SETUP();
    ...
    if ( chargerwallet_data )
    {
        if ( !emmc_wrapper_status.is_mounted_chargerwallet_data )
            return false;
        strcpy(path_buff, "0:");
