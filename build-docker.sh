...
REPOSITORY="https://github.com/chargerwallet/chargerwallet-firmware.git"
CONTAINER_NAME="lyxyx/chargerwallet-build"
FAMILY=""
BRANCH=""
TAG=""
...
# pull docker image
docker pull lyxyx/chargerwallet-build
...
cd /tmp
git clone "$REPOSITORY" chargerwallet-firmware
...
cat <<EOF > "build/$PULL_SCRIPT_NAME"
cd /tmp
git clone -b mini "$REPOSITORY" chargerwallet-firmware
cd chargerwallet-firmware
git checkout $TAG
chmod a+x /local/build/$BUILD_SCRIPT_NAME
/root/.nix-profile/bin/nix-shell --run /local/build/$BUILD_SCRIPT_NAME
...
set -e -o pipefail
cd /tmp
git clone -b bixin_dev "$REPOSITORY" chargerwallet-firmware
cd chargerwallet-firmware
git checkout $TAG
chmod a+x /local/build/$BUILD_SCRIPT_NAME
/root/.nix-profile/bin/nix-shell --run /local/build/$BUILD_SCRIPT_NAME
